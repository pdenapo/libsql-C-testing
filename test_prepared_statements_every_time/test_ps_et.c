...
int insert_person(const char* name,int age,libsql_connection_t conn,const char **out_err_msg)
{
		libsql_stmt_t insert_stmt;
		const char* insert_sql = "INSERT INTO Persons VALUES (?,?)";
		
		int retval=libsql_prepare(conn, insert_sql, &insert_stmt, out_err_msg);
		if (retval != 0) return retval;
	
		fprintf(stderr,"insert_person (%s,%d)\n",name,age);
		retval=libsql_bind_string(insert_stmt,1, name, out_err_msg);
		if (retval != 0) return retval;
...
int main(int argc, char *argv[])
{
 ...
 const char* create_table="CREATE TABLE Persons(Name TEXT,Age INTEGER);";
  ...
  if (retval != 0) {
	...
	retval= insert_person("Paul",20,conn,&err);
	if (retval != 0) {
		fprintf( stderr, "%s\n", err);
		goto quit;
	}
	retval= insert_person("Laura",30,conn,&err);
	if (retval != 0) {
		fprintf( stderr, "%s\n", err);
		goto quit;
	}
